...
public class LabManager : MonoBehaviour {
	...
	private ReadBlueData blueRotator;
	...
	void Start () {
		...
		blueRotator = GameObject.Find ("BlueDataReader").GetComponent<ReadBlueData>();
	...
	IEnumerator targetLoop() {
		for (int l = 0; l < 2; l++) {
			for (int i = 0; i < trials.Count; i++) {
				//teleport here
				ui.text = "Schauen Sie geradeaus und teilen Sie dem Versuchsleiter mit, das er ihre Sicht zurückstellen soll.";
				yield return new WaitUntil (() => GvrController.Recentered);
				resetSound.PlayOneShot (resetSound.clip, 1);
				ui.text = "";
				...
				for (int j = 0; j < 5; j++) {
					...
					yield return new WaitUntil (() => blueRotator.buttonDown());//test if i can use this to jump to next phase
					setInput(blueRotator.rotationInput());
	...
	private string getPath(){
		...
		#if UNITY_EDITOR
		return Application.dataPath +"/CSV/"+"Saved_data_A_"+subject_date_time+".csv";
		#elif UNITY_ANDROID
		return Application.persistentDataPath+"Saved_data_A_"+subject_date_time+".csv";
		#endif
	}
